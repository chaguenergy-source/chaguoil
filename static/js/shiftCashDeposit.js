...
const renderPaymentDetails = d =>{
    const {payments,rname,id} = d,
          modalTitle = lang('Cash kwenda Benki - ','Cash Deposit - ') + `<span class="bluePrint">${rname}</span>`
    let tr = '',toFocus = ''; 
    // filter payments
    const {st} = filters();
   ...
   if(customerId){
        ...
        filteredPayments = filteredPayments.filter(p => !p.admin_approval);
        ...
        toFocus = `  
                <div id="selectedCustomerCard" class="card mb-3" >
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div>
                            <h6 class="mb-0">${lang('Miamala ambayo Haijahakikiwa: ','Unapproved Transactions: ')}</h6>
                        </div>
                        <button type="button" data-report=${id} class="moreDetails btn-lg largerfont btn btn-close" id="closeCustomerCard" aria-label="Close">&times;</button>
                    </div>
                </div>`
    }
    ...
    filteredPayments.forEach(p=>{
        ...
        tr += `<tr class="smallFont  cursor-pointer" data-custid="${p.customer_id}" data-report=${id}  >
                   ...
                   <td data-custid="${p.customer_id}" data-report=${id} class="customerLink" >${formatNumber(p.After) || 0}</td>
                    ...
                    <td>
                          <span data-custid="${p.customer_id}" data-report=${id} class="customerLink">${p.admin_approval ? lang('Imethibitishwa','Approved') : lang('Haijathibitishwa','Unapproved')}</span>
