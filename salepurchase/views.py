...
def  lipaInvo(request):
      ...
      if request.method == 'POST':
            try:
                  ...
                  cust = None
                  if pall:
                     cust = wateja.objects.get(pk=value)
                     bill = fuelSales.objects.filter(by__Interprise=shell,customer=value,payed__lt=F('amount'))
                     kutoka = cust.jina
    ...
    if request.method == 'POST':
        try:
            ...
            if( useri.admin or manager) and not general:
                ...
                sales.date = datetime.datetime.now(tz=timezone.utc)
